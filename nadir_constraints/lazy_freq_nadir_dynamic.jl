function lazy_freq_nadir_v2(
    ...
    if constraint_mode == "Dynamic"
        ...
        @variable(model, temp_prod[1:instance.time, 1:length(thermal_units)])
        ...
        function dynamic1_callback_function(cb_data)
            ...
            for t in 1:instance.time
                ...
                #const_par = thermal_units[g].max_power[T]/sum(thermal_units[g].max_power[1] for g in eachindex(thermal_units))
                temp_RT = sum(R_T[g] * callback_value(cb_data, model[:is_on][thermal_units[g].name, t]) for g in eachindex(thermal_units)) - sum(R_T[ind_max[i]] for i in 1:n_cont)
                temp_FT = sum(F_T[g] * callback_value(cb_data, model[:is_on][thermal_units[g].name, t]) for g in eachindex(thermal_units)) - sum(F_T[ind_max[i]] for i in 1:n_cont)
                temp_HT = sum(H_T[g] * callback_value(cb_data, model[:is_on][thermal_units[g].name, t]) for g in eachindex(thermal_units)) - sum(H_T[ind_max[i]] for i in 1:n_cont)
                temp_DT = sum(D_T[g] * callback_value(cb_data, model[:is_on][thermal_units[g].name, t]) for g in eachindex(thermal_units)) - sum(D_T[ind_max[i]] for i in 1:n_cont)
                ...
                RD = [(DData.D[g] + DData.K[g] / DData.R[g]) * DData.pu[g] for g in 1:length(thermal_units)]
                ...
                if viol_rocof > 1e-4
                    ...
                    con = @build_constraint(sum(H_T[g] * model[:is_on][thermal_units[g].name, t] for g in eachindex(thermal_units))
                                            -
                                            sum(H_T[ind_max[i]] * model[:is_on][thermal_units[ind_max[i]].name, t] for i in 1:n_cont)
                                            -
                ...
                viol_ss = -(sum(callback_value(cb_data, model[:is_on][thermal_units[g].name, t]) * RD[g] for g in eachindex(thermal_units))
                            -
                ...
                if viol_ss > 1e-4
                    ...
                    con = @build_constraint(sum(RD[g] * model[:is_on][thermal_units[g].name, t] for g in eachindex(thermal_units))
                                            -
                                            sum(RD[ind_max[i]] * model[:is_on][thermal_units[ind_max[i]].name, t] for i in 1:n_cont)
                                            -
                ...
                if viol_nadir > 1e-4
                    #Dumb way
                    # temp_on,temp_off = zeros((length(thermal_units)))
                    #  for g in eachindex(thermal_units)
                    #      if callback_value(cb_data,model[:is_on][thermal_units[g].name,t]) == 0
                    #          temp_off[g]=1
                    #      else
                    #          temp_on[g]=1
                    #      end
                    ...
                    con = @build_constraint((scale_H * (sum(H_T[g] * model[:is_on][thermal_units[g].name, t] for g in eachindex(thermal_units)) -
                                                        sum(H_T[ind_max[i]] * model[:is_on][thermal_units[ind_max[i]].name, t] for i in 1:n_cont)) +
                                             scale_R * (sum(R_T[g] * model[:is_on][thermal_units[g].name, t] for g in eachindex(thermal_units)) -
                                                        sum(R_T[ind_max[i]] * model[:is_on][thermal_units[ind_max[i]].name, t] for i in 1:n_cont)) +
                                             scale_F * (sum(F_T[g] * model[:is_on][thermal_units[g].name, t] for g in eachindex(thermal_units)) -
                                                        sum(F_T[ind_max[i]] * model[:is_on][thermal_units[ind_max[i]].name, t] for i in 1:n_cont)) +
                                             scale_D * (sum(D_T[g] * model[:is_on][thermal_units[g].name, t] for g in eachindex(thermal_units)) -
                                                        sum(D_T[ind_max[i]] * model[:is_on][thermal_units[ind_max[i]].name, t] for i in 1:n_cont)) + const_val) * 0.6 / 60
                                            -
