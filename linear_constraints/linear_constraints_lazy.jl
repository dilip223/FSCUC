function linear_constraints_lazy(
    ...
    P_tot = sum(g.max_power[1] for g in thermal_units)

    if constraint_mode == "Constant"
        #Constant model with k generator failures
        max_deltaP = sort([DData.pu[g] for g in 1:length(thermal_units)], rev=true)[1:n_cont]
        ind_loss = sortperm([DData.pu[g] for g in 1:length(thermal_units)], rev=true)[1:n_cont]
        ...
        function constant_callback_function(cb_data)
            ...
            for t in 1:instance.time
                ...
                viol_rocof = -(sum(H[g] * callback_value(cb_data, model[:is_on][thermal_units[g].name, t]) for g in eachindex(thermal_units)) - H_loss - sum(max_deltaP) * 60)

                if viol_ss > 1e-4
                    con = @build_constraint(sum(model[:is_on][thermal_units[g].name, t] * RD[g] for g in eachindex(thermal_units))
                                            -
    ...
    if constraint_mode == "Static"
        ...
        function static_callback_function(cb_data)
            ...
            for t in 1:instance.time
                ...
                viol_rocof = -(sum(H[i] * callback_value(cb_data, model[:is_on][thermal_units[i].name, t]) for i in eachindex(thermal_units))
                               -
                ...
                if viol_ss > 1e-4
                    ...
                    con = @build_constraint(sum(RD[g] * model[:is_on][thermal_units[g].name, t] for g in eachindex(thermal_units))
                                            ...
                                            -
                                            60 * sum(DData.pu[ind_max[i]] * model[:is_on][thermal_units[ind_max[i]].name, t] for i in 1:n_cont) / 0.3 >= 0)
                ...
                if viol_rocof > 1e-4
                    ...
                    con = @build_constraint(sum(H[g] * model[:is_on][thermal_units[g].name, t] for g in eachindex(thermal_units))
                                            ...
                                            -
                                            60 * sum(DData.pu[ind_max[i]] * model[:is_on][thermal_units[ind_max[i]].name, t] for i in 1:n_cont) >= 0)
...
function linear_constraints_lazy(
    ...
    thermal_units = instance.scenarios[1].thermal_units
    H = DData.H .* DData.pu
    RD = [(DData.D[g] + DData.K[g] / DData.R[g]) * DData.pu[g] for g in eachindex(thermal_units)]
    ...
    if constraint_mode == "Constant"
        ...
        ind_loss = sortperm([thermal_units[g].max_power[1] for g in eachindex(thermal_units)] ./
                            P_tot, rev=true)[1:n_cont]
        RD_loss = sum(RD[ind_loss[i]] for i in 1:n_cont)
        H_loss = sum(H[ind_loss[i]] for i in 1:n_cont)
        ...
        function constant_callback_function(cb_data)
            ...
            for t in 1:instance.time
                ...
                viol_rocof = -(sum(H[g] * callback_value(cb_data, model[:is_on][thermal_units[g].name, t]) for g in eachindex(thermal_units))
                               -
                               H_loss
                               -
                               sum(max_deltaP) * 60
            ...
            for t in 1:instance.time
                ...
                if viol_ss > 1e-4
                    ...
                    con = @build_constraint(sum(RD[g] * model[:is_on][thermal_units[g].name, t] for g in eachindex(thermal_units))
                                            ...
                                            sum(RD[ind_max[i]] * model[:is_on][thermal_units[ind_max[i]].name, t] for i in 1:n_cont)
                                            -
                                            60 / 0.3 * sum(DData.pu[ind_max[i]] * model[:is_on][thermal_units[ind_max[i]].name, t] for i in 1:n_cont)
                                            +
                                            60 / 0.3 * sum(model[:P_dis][s, t] for s in 1:length(ESSData.ess_bus)) ./ P_tot >= 0)
                ...
                if viol_rocof > 1e-4
                    ...
                    con = @build_constraint(sum(H[g] * model[:is_on][thermal_units[g].name, t] for g in eachindex(thermal_units))
                                            ...
                                            sum(H[ind_max[i]] * model[:is_on][thermal_units[ind_max[i]].name, t] for i in 1:n_cont)
                                            -
                                            60 * sum(DData.pu[ind_max[i]] * model[:is_on][thermal_units[ind_max[i]].name, t] for i in 1:n_cont)
                                            +
                                            60 * sum(model[:P_dis][s, t] for s in 1:length(ESSData.ess_bus)) ./ P_tot >= 0)
            ...
            for t in 1:instance.time
                ...
                if viol_ss > 1e-4
                    ...
                    con = @build_constraint(sum(RD[g] * model[:is_on][thermal_units[g].name, t] for g in eachindex(thermal_units))
                                            ...
                                            sum(RD[ind_max[i]] * model[:is_on][thermal_units[ind_max[i]].name, t] for i in 1:n_cont)
                                            -
                                            60 / 0.3 * sum(temp_prod[t, ind_max[i]] for i in 1:n_cont) ./ sum(thermal_units[i].max_power[1] for i in eachindex(thermal_units))
                                            +
                                            60 / 0.3 * sum(model[:P_dis][s, t] for s in 1:length(ESSData.ess_bus)) ./ P_tot >= 0)
                ...
                if viol_rocof > 1e-4
                    ...
                    con = @build_constraint(sum(H[g] * model[:is_on][thermal_units[g].name, t] for g in eachindex(thermal_units))
                                            ...
                                            sum(H[ind_max[i]] * model[:is_on][thermal_units[ind_max[i]].name, t] for i in 1:n_cont)
                                            -
                                            60 * sum(temp_prod[t, ind_max[i]] for i in 1:n_cont) ./ sum(thermal_units[i].max_power[1] for i in eachindex(thermal_units))
                                            +
                                            60 / 0.3 * sum(model[:P_dis][s, t] for s in 1:length(ESSData.ess_bus)) ./ P_tot >= 0)
