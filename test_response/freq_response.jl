function freq_test(
   ...
   for t in 1:T
      ...
      pow_mat = [(thermal_units[g].min_power[1] +
                  value(model[:prod_above][instance.scenarios[1].name, thermal_units[g].name, t])) .*
                 value(model[:is_on][thermal_units[g].name, t])
                 for g in eachindex(thermal_units)]
      pow_max = sort(pow_mat, rev=true)[1:n_cont]
      x = sum(pow_mat)
      ind_max = sortperm(pow_mat, rev=true)[1:n_cont]
      gen_lost_name = [thermal_units[ind_max[g]].name for g in 1:n_cont]
      println("Lost generators = $gen_lost_name Lost Power = $pow_max, Total Power =$x, Time = $t")
      pow_max = sum(pow_max) ./ P_tot
      temp = [value(model[:is_on][thermal_units[g].name, t])
              for g in eachindex(thermal_units)]
      temp[ind_max] .= 0

      for g in 1:length(thermal_units)
         on_status = temp[g]
         #const_par = thermal_units[g].max_power[T]/sum(thermal_units[g].max_power[1] for g in 1:length(thermal_units))
